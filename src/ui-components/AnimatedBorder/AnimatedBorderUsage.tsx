...
export default function AnimatedBorderUsage(props: any) {
  ...
  return (
    <div className="flex items-center justify-center w-full h-full min-h-[400px] p-8 bg-[#030303] rounded-2xl">
      <AnimatedBorder className="w-full max-w-2xl" {...props}>
        <div ref={containerRef} className="flex flex-col p-4 w-full text-left">
          ...
          <div className="relative w-full min-h-[30px] flex-shrink-0">
            ...
            {!inputValue && (
              <div className="absolute top-0 left-0 flex gap-1.5 pointer-events-none text-lg text-zinc-500 font-medium">
            ...
            <textarea
              ...
              placeholder={inputValue ? '' : ' '} // Space keeps height logic but hides native placeholder
          ...
          <div ref={footerRef} className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
            ...
            <div className="flex items-center gap-2">
              <button
                style={{ opacity: 0 }}
                className="footer-anim-item flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.03] hover:bg-white/[0.08] active:bg-white/[0.12] text-zinc-400 hover:text-white rounded-full text-xs font-medium border border-white/[0.05] transition-all cursor-pointer"
              >
            ...
            <div>
              <button
                style={{ opacity: 0 }}
                className={`footer-anim-item flex items-center justify-center w-8 h-8 rounded-full transition-all cursor-pointer ${inputValue.trim()
                    ? 'bg-white text-black hover:scale-105 active:scale-95'
                    : 'bg-white/[0.06] text-zinc-500 border border-white/[0.05]'
                  }`}
