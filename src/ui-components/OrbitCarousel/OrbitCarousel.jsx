...
export default function OrbitCarousel({ items = defaultItems, radius = 220, duration = 15, cardWidth = 120, cardHeight = 160, className = "", }) {
    ...
    return (<div className={`${styles.wrapper} ${className}`} style={{
      ...
      {/*
        ...
        */}
      <motion.div key={`${d}-${r}-${cw}-${ch}`} className={styles.orbitContainer} animate={rotation} transition={transition}>
        {safeItems.map((item, index) => {
            const angle = (index / safeItems.length) * (Math.PI * 2);
            const x = Math.cos(angle) * r;
            const y = Math.sin(angle) * r;
            return (<div key={item.id} className={styles.cardPositioner} style={{
                    transform: `translate(${x}px, ${y}px)`,
                }}>
              <motion.div className={styles.card} animate={counterRotation} transition={transition} whileHover={{
                    scale: 1.15,
                    zIndex: 50,
                    transition: { duration: 0.2, repeat: 0 }
                }}>
                {item.image ? (<img src={item.image} alt={item.title} className={styles.cardImage}/>) : (<div className={styles.cardFallback} style={{ background: item.color }}/>)}
                {item.title && (<div className={styles.cardOverlay}>
