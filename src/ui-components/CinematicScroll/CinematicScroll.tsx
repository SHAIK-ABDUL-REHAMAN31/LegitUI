...
export interface CinematicScrollProps {
  panels?: CinematicPanelProps[];
  className?: string;
}

const defaultPanels: CinematicPanelProps[] = [
  {
    id: "01",
    title: "ASCEND",
...
export default function CinematicScroll({
  ...
  useEffect(() => {
    ...
    const panelsElems = gsap.utils.toArray(containerRef.current?.querySelectorAll(`.${styles.panel}`) || []) as HTMLElement[];
    ...
    panelsElems.forEach((panel, i) => {
      const img = panel.querySelector(`.${styles.image}`) as HTMLElement;
      const inner = panel.querySelector(`.${styles.inner}`) as HTMLElement;
