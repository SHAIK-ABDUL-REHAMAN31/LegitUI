...
export interface DepthTextProps {
    /** The text to display */
    text?: string;
    /** Number of text layers to simulate depth */
    layers?: number;
    /** Distance in pixels between each layer */
    depth?: number;
    /** Font size */
    ...
    /** Font family */
    fontFamily?: string;
    /** Color of the frontmost text */
    textColor?: string;
    /** Color of the trailing shadow layers */
    shadowColor?: string;
...
export default function DepthText({
    ...
    useEffect(() => {
        ...
        const handleMouseLeave = () => {
            ...
            gsap.to(scene, {
                ...
                ease: "power3.out",
                onComplete: () => {
                    if (!isHovering) driftTimeline.restart();
                }
            });
        };
    ...
    for (let i = 0; i < layers; i++) {
        ...
        layerElements.push(
            <div 
                key={i}
                className={`${styles.textLayer} ${i === 0 ? styles.front : ''}`}
                style={{
