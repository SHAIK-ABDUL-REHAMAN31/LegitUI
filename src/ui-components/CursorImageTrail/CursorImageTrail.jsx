...
export default function CursorImageTrail({ images, renderCount = 15, children, }) {
    ...
    const repeatedImages = Array.from({ length: renderCount }).map((_, i) => images[i % images.length]);
    return (<div className={styles.container}>
      <div className={styles.content}>{children}</div>

      {repeatedImages.map((src, i) => (<img key={i} ref={(el) => {
                if (el)
                    imagesRef.current[i] = el;
            }} src={src} className={styles.trailImage} alt={`trail-${i}`} style={{ zIndex: renderCount - i }} // Head (0) has highest z-index
        />))}
    </div>);
}
