...
function DockItem({ icon, label, onClick, mouseX, isActive }) {
    ...
    return (<motion.div ref={ref} style={{ width }} onMouseEnter={() => setIsHovered(true)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} onClick={onClick} className={`${styles.dockItem} ${isActive ? styles.active : ""}`}>
      <motion.div className={styles.dockIconContainer} style={{
            width: iconSize,
            height: iconSize,
            y: ySpring,
            x: springMagX
      ...
      <AnimatePresence>
        {isHovered && (<motion.div style={{ y: tooltipYSpring }} className={styles.dockTooltipWrapper}>
            <motion.div initial={{ opacity: 0, y: 10, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 5, scale: 0.8 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className={styles.dockTooltip}>
              {label}
            </motion.div>
          </motion.div>)}
      </AnimatePresence>
...
export default function MagneticDock({ items, activeItem, className = "" }) {
    const mouseX = useMotionValue(Infinity);
    return (<div className={`${styles.dockWrapper} ${className}`}>
      <motion.div className={styles.dockContainer} onMouseMove={(e) => mouseX.set(e.pageX)} onMouseLeave={() => mouseX.set(Infinity)}>
        {items.map((item, idx) => (<DockItem key={idx} icon={item.icon} label={item.label} onClick={item.onClick} mouseX={mouseX} isActive={activeItem === item.label}/>))}
      </motion.div>
    </div>);
}
