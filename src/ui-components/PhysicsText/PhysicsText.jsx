...
export default function PhysicsText({ text = "Craft stunning interactive user interfaces with LegitUI", splitMode = "words", fontSize = "clamp(1.5rem, 4vw, 2.5rem)", fontWeight = 600, fontFamily = "'Inter', system-ui, sans-serif", textColor = "#ffffff", gravity = 1, bounciness = 0.6, friction = 0.1, className = "", capsuleBg = "#000000", capsuleBorder = "rgba(255, 255, 255, 0.4)", showSparkles = false, }) {
    ...
    useEffect(() => {
        ...
        const bodies = items.map((_, i) => {
            ...
            const body = Matter.Bodies.rectangle(startX, startY, bbox.width, bbox.height, {
    ...
    return (<div ref={containerRef} className={`${styles.container} ${className}`}>
            {items.map((item, i) => (<div key={`${item}-${i}`} ref={(el) => {
                textRefs.current[i] = el;
            }} className={styles.capsule} style={{
                opacity: 0,
                ...
                borderColor: capsuleBorder,
                    ...
                    {showSparkles && (<div className={styles.sparklesContainer}>
