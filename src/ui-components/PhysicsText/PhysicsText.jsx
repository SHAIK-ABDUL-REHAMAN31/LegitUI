...
export default function PhysicsText({ text = "DRAG ME AROUND PLAYFUL GRAVITY BOUNCING", splitMode = "words", fontSize = "clamp(2rem, 6vw, 5rem)", fontWeight = 900, fontFamily = "'Inter', system-ui, sans-serif", textColor = "#ffffff", gravity = 1, bounciness = 0.6, friction = 0.1, className = "", }) {
    ...
    useEffect(() => {
        ...
        const bodies = items.map((_, i) => {
            ...
            const body = Matter.Bodies.rectangle(startX, startY, bbox.width * 0.95, // slight tolerance so they can pack tighter
    ...
    return (<div ref={containerRef} className={`${styles.container} ${className}`}>
            <svg ref={svgRef} className={styles.svg}>
                {items.map((item, i) => (<text key={`${item}-${i}`} ref={(el) => {
                textRefs.current[i] = el;
            }} className={styles.textParticle} x={0} y={0} fontSize={fontSize} fontWeight={fontWeight} fontFamily={fontFamily} fill={textColor} textAnchor="middle" dominantBaseline="central" style={{ opacity: 0 }}>
                        {item}
                    </text>))}
            </svg>
        </div>);
