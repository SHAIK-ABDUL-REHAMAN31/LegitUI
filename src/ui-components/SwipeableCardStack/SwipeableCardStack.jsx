...
export default function SwipeableCardStack({ cards = defaultCards, swipeSpeed = 0.5, cardWidth = 240, cardHeight = 340, rotationOffset = 4, maxVisibleCards = 3, enableWheelSwipe = true, className = "", onSwipe, onReset, }) {
    ...
    const swipeTopCard = useCallback((direction) => {
    ...
    useEffect(() => {
        activeCards.forEach((card, index) => {
            ...
            if (index === 0 && dragInfo.current.isDragging)
                return;
    ...
    return (<div className={styles.stackWrapper} ref={containerRef}>
            {/* Left Control Button */}
            {activeCards.length > 0 && (<button className={`${styles.controlBtn} ${styles.controlBtnLeft}`} onClick={() => swipeTopCard("left")} title="Swipe Left">
                    <ArrowLeft size={18}/>
                </button>)}
        ...
        {[...activeCards].reverse().map((card) => {
            ...
            {activeCards.length > 0 && (<button className={`${styles.controlBtn} ${styles.controlBtnRight}`} onClick={() => swipeTopCard("right")} title="Swipe Right">
                    <ArrowRight size={18}/>
                </button>)}
        </div>);
}
