...
interface ScrollCircleTransitionProps {
  /** Custom content for Section 1. If not provided, a premium default hero section is shown. */
  section1Content?: React.ReactNode;
  /** Custom content for Section 2. If not provided, a premium default features grid is shown. */
  section2Content?: React.ReactNode;
  /** Array of hex or RGB colors for the expanding circles. Must have at least 1 color. The last color becomes the background color of Section 2. */
  colors?: string[];
  /** Animation duration in seconds for each individual circle expansion. @default 2.4 */
  animationDuration?: number;
  /** Stagger delay in seconds between consecutive circle expansions. @default 0.4 */
  staggerDelay?: number;
  /** Additional CSS class names for the root container. */
  className?: string;
  /** Callback fired when transition starts. */
  onTransitionStart?: (targetSection: 1 | 2) => void;
  /** Callback fired when transition completes. */
  onTransitionComplete?: (activeSection: 1 | 2) => void;
}
...
const ScrollCircleTransition: React.FC<ScrollCircleTransitionProps> = ({
  section1Content,
  section2Content,
  ...
  return (
    ...
    >
      {/* ── SECTION 1 ── */}
      <div
        className={`${styles.section1} ${activeSection === 2 ? styles.section1Hidden : ''
          }`}
      >
        {section1Content ? (
          section1Content
        ...
        {colors.map((color, index) => {
          ...
          return (
            <motion.div
              ...
              initial={{ scale: 0 }}
              animate={{ scale: isTransitioning ? 1 : 0 }}
              transition={{
      ...
      <div
        ref={section2Ref}
        className={`${styles.section2} ${activeSection === 2 ? styles.section2Visible : ''
          }`}
      >
        {activeSection === 2 && (
          section2Content ? (
            ...
            <div className={styles.sec2Content}>
              ...
              >
                <div className={styles.hugeTextMask}>
                  <motion.h2
                    className={styles.hugeSingleLine}
                    variants={{
                      hidden: { y: '105%' },
                      visible: { y: 0 }
                    }}
