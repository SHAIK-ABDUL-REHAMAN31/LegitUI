    ...
    useEffect(() => {
        ...
        Promise.all(images.map((img) => new Promise((res) => {
            ...
            el.onerror = () => {
                ...
                ctx.fillText(img.title, W / 2, H / 2);
...
function CarouselScene({ images, curvature, cardWidth, cardHeight, gap, speed, }) {
    const meshes = useRef([]);
    ...
    useFrame((_, delta) => {
        ...
        // Wrap
        while (offsetRef.current < -totalW)
    ...
    for (let copy = -1; copy <= 1; copy++) {
        for (let i = 0; i < n; i++) {
            ...
            cards.push(<mesh key={`${copy}-${i}`} ref={(el) => { meshes.current[idx] = el; }}>
...
/* ─── Main export ─── */
export default function ScrollGallery({ images, curvature = 10, cardWidth = 2, cardHeight = 2, gap = 0.35, speed = 2.5, fov = 55, backgroundColor = '#ffffff', textColor = '#111111', }) {
    return (<div className={styles["gallery3d-wrapper"]} style={{ background: backgroundColor }}>
            <div className={styles["gallery3d-bg"]}/>
            <div className={styles["gallery3d-vignette"]}/>
            <div className={styles["gallery3d-canvas"]}>
                <Canvas camera={{ position: [0, 0, 5], fov, near: 0.1, far: 100 }} gl={{ antialias: true, alpha: true }}>
                    <Suspense fallback={null}>
                        <CarouselScene images={images} curvature={curvature} cardWidth={cardWidth} cardHeight={cardHeight} gap={gap} speed={speed}/>
                    </Suspense>
                </Canvas>
            </div>
