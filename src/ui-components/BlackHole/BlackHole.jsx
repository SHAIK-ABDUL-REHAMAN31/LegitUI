...
function AccretionDisk({ radius, speed, colorInner, colorMid, colorOuter }) {
    ...
    return (<mesh ref={meshRef} rotation={[-Math.PI / 2.5, 0, 0]}>
      <planeGeometry args={[radius * 12, radius * 12, 1, 1]}/>
      <shaderMaterial ref={materialRef} transparent side={THREE.DoubleSide} vertexShader={AccretionDiskShader.vertexShader} fragmentShader={AccretionDiskShader.fragmentShader} uniforms={uniforms} blending={THREE.AdditiveBlending}/>
    </mesh>);
}
function CinematicBlackHoleContent({ speed, radius, starsCount, colorInner, colorMid, colorOuter }) {
    const { viewport } = useThree();
    return (<>
      <color attach="background" args={['#010103']}/>
      <PerspectiveCamera makeDefault position={[0, 1.5, 4.5]} fov={75}/>

      <Stars radius={100} depth={50} count={starsCount} factor={4} saturation={0} fade speed={speed * 0.5}/>
      ...
      <Float speed={speed * 2} rotationIntensity={0.5} floatIntensity={0.5}>
        <group scale={viewport.width / 6}>
          ...
          <AccretionDisk radius={radius} speed={speed} colorInner={colorInner} colorMid={colorMid} colorOuter={colorOuter}/>
...
export default function BlackHole({ containerClassName = '', speed = 1.0, radius = 0.3, starsCount = 1500, colorOuter = '#4c1d95', colorMid = '#a855f7', colorInner = '#fff0ff', }) {
    return (<div className={containerClassName} style={{ width: '100%', height: '100%', minHeight: '400px' }}>
      <Canvas gl={{ antialias: false, stencil: false, depth: true }} dpr={[1, 2]}>
        <CinematicBlackHoleContent speed={speed} radius={radius} starsCount={starsCount} colorInner={colorInner} colorMid={colorMid} colorOuter={colorOuter}/>
      </Canvas>
    </div>);
}
