...
const Word = ({ children, progress, range }) => {
    ...
    return (<span className={styles.wordWrapper}>
      <motion.span className={styles.word} style={{ opacity, y, filter }}>
        {children}
      </motion.span>
    </span>);
};
const ScrollRevealText = ({ text = "Scroll to reveal this cinematic text scrub animation. It dynamically maps opacity, vertical position, and clip-path directly to your scroll progress for a buttery smooth reading experience.", className = "", fontSize = "clamp(2rem, 5vw, 4rem)", color = "#ffffff", }) => {
