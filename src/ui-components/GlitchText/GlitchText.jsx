...
export default function GlitchText({ text = "CYBERPUNK", fontSize = "clamp(3rem, 10vw, 8rem)", fontWeight = "900", fontFamily = "'Inter', system-ui, sans-serif", className = "", intensity = 1, }) {
    ...
    return (<div ref={containerRef} className={`${styles.container} ${className}`} style={{ fontSize, fontWeight, fontFamily }}>
            ...
            <svg className={styles.svgFilter}>
                <defs>
                    <filter id={`glitch-displacement-${filterId}`} colorInterpolationFilters="sRGB">
                        <feTurbulence ref={filterRef} type="fractalNoise" baseFrequency="0.05 0.9" numOctaves="2" result="noise"/>
            ...
            {/* The Text Layers */}
            <div className={styles.textWrapper} style={{ filter: `url(#glitch-displacement-${filterId})` }}>
