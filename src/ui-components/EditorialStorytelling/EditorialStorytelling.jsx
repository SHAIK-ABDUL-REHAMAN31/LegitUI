...
export default function EditorialStorytelling({ chapters, }) {
    ...
    useEffect(() => {
        ...
        const splits = textRefs.current.map((el) => {
            ...
            if (titleEl) {
                return new SplitType(titleEl, { types: "chars,words" });
            }
            return null;
        });
        ...
        chapters.forEach((_, index) => {
            ...
            if (index === 0) {
                ...
                tl.to(imgChild, { scale: 1, filter: "blur(0px)", duration: 1.5, ease: "power2.out" }, chapterLabel);
                ...
                // Subtitle float in
                if (desc) {
                    tl.to(desc, { opacity: 1, y: 0, duration: 1, ease: "power2.out" }, `${chapterLabel}+=0.5`);
                }
            }
            // Fade out current chapter when moving to the next
            if (index < chapters.length - 1) {
                const nextChapterLabel = `chapter${index + 1}`;
                tl.to(textEl, { opacity: 0, y: -50, duration: 0.5 }, `${nextChapterLabel}-=0.5`);
    ...
    return (<div className={styles.container} ref={containerRef}>
      <div className={styles.leftContent}>
        {chapters.map((chapter, i) => (<div key={i} className={styles.chapterText} ref={(el) => {
                if (el)
      ...
      <div className={styles.rightContent}>
        {chapters.map((chapter, i) => (<div key={i} className={styles.imageContainer} ref={(el) => {
                if (el)
