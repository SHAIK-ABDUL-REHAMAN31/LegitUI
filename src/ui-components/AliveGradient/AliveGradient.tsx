...
/* ─── Fragment Shader ───
   Alive Fluid Glow Background:
   - High-fidelity FBM domain warping for fluid drift.
   - Parabolic warm wave representing flame (left/middle).
   - Smoothstep S-curve cool wave representing plasma (right).
   - Side-biased iridescent transition edge.
   - Soft vignette and subtle breathing.
─── */
const FRAG = /* glsl */ `
...
void main() {
  ...
  // Domain warp using 3-octave FBM for fluid flow look
  vec2 warp = vec2(
  ...
  // Flow 1: Warm Flame (Parabola centered around x=0.32)
  ...
  // Flow 2: Cool Cosmic Plasma (S-Curve sloping up to the right)
  ...
  // Iridescent green edge transition (one side of the blue flow only)
  ...
  // Add Cool Flow + Iridescent green edge
...
export interface AliveGradientProps {
  className?: string;
  /** Deep background void color. @default "#020205" */
  background?: string;
  /** Warm outer orange glow color. @default "#ea580c" */
  color1?: string;
  /** Warm inner gold core color. @default "#eab308" */
  color2?: string;
  /** Cool outer blue glow color. @default "#2563eb" */
  color3?: string;
  /** Cool inner white core color. @default "#ffffff" */
  color4?: string;
  /** Iridescent green edge color. @default "#10b981" */
  color5?: string;
  /** Animation flow speed. @default 0.35 */
  speed?: number;
  /** Glow brightness intensity. @default 1.0 */
  intensity?: number;
  /** Organic fluid warp distortion strength. @default 0.4 */
  distortion?: number;
  /** Glow radius size multiplier. @default 1.0 */
  glowSize?: number;
...
export default function AliveGradient({
  ...
  // Keep a ref of values to stay updated inside requestAnimationFrame loop
