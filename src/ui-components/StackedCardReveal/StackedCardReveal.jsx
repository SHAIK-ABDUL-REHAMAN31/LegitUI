...
const Card = ({ card, index, progress, totalCards }) => {
    ...
    return (<div className={styles.cardContainer} style={{ top: `calc(10vh + ${index * 30}px)` }}>
...
export const StackedCardReveal = ({ cards = defaultCards, containerHeight = "100vh" }) => {
    ...
    return (<div className={styles.wrapper} ref={containerRef}>
      {cards.map((card, i) => (<Card key={card.id} card={card} index={i} progress={scrollYProgress} totalCards={cards.length}/>))}
    </div>);
};
