...
export interface MarqueeRow {
  images: string[];
  speed?: number; // Base speed, negative values reverse the direction
}
    ...
    rowsRef.current.forEach((rowElem, i) => {
      ...
      if (direction < 0) {
        ...
        tween = gsap.fromTo(
          rowElem,
          { x: 0 },
          { x: -trackWidth, duration: baseDuration, ease: "none", repeat: -1 }
        );
      } else {
        // Move Right
        tween = gsap.fromTo(
          rowElem,
          { x: -trackWidth },
          { x: 0, duration: baseDuration, ease: "none", repeat: -1 }
        );
      }
      ...
      gsap.fromTo(
        rowElem,
        { opacity: 0, xPercent: startXPercent },
        {
          opacity: 1,
          xPercent: 0,
          duration: 3,
          ...
          >
            ...
            <div className={styles.track}>
              {row.images.map((src, idx) => (
                <div key={`a-${idx}`} className={styles.imageBox}>
            ...
            <div className={styles.track}>
              {row.images.map((src, idx) => (
                <div key={`b-${idx}`} className={styles.imageBox}>
                  <img src={src} alt="marquee" className={styles.image} />
                </div>
              ))}
            </div>
