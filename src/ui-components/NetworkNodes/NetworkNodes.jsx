...
export default function NetworkNodes({ mainNodeIcon, nodes = [], particleColor = "#ffffff", lineColor = "rgba(255, 255, 255, 0.2)", particleSpeed = 3, className = "", }) {
    ...
    return (<div ref={containerRef} className={`${styles.container} ${className}`}>
      {containerSize.width > 0 && (<svg className={styles.svgLayer}>
          {nodes.map((node, i) => {
                ...
                return (<g key={`path-${node.id}`}>
                <path d={pathData} className={styles.line} style={{ stroke: lineColor }}/>
                <motion.path d={pathData} className={styles.particle} style={{ stroke: particleColor }} pathLength="1" strokeDasharray="0.03 1.5" initial={{ strokeDashoffset: 1 }} animate={{ strokeDashoffset: 0 }} transition={{
      ...
      <div className={styles.mainNodeWrapper}>
        {mainNodeIcon || (<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {containerSize.width > 0 &&
            nodes.map((node) => {
                ...
                return (<div key={node.id} className={styles.subNodeWrapper} style={{ left: `${x}px`, top: `${y}px` }}>
              {node.icon}
            </div>);
            })}
    </div>);
