...
function StepNode({ step, index, totalSteps, scrollYProgress }) {
    ...
    return (<div className={`${styles.stepRow} ${isLeft ? styles.rowLeft : styles.rowRight}`}>
      <motion.div className={styles.connectorLine} style={{ opacity: lineOpacity }}/>
      
      <motion.div className={styles.stepCard} style={{ opacity, filter, boxShadow }}>
...
export default function TimelineSteps({ steps, className = "" }) {
    ...
    return (<div className={`${styles.container} ${className}`}>
      <div className={styles.timelineWrapper} ref={containerRef}>
        <div className={styles.timelineLine}>
          <motion.div className={styles.timelineDot} style={{ top: dotTop }}/>
          <motion.div className={styles.timelineTrail} style={{ height: trailHeight }}/>
        </div>

        <div className={styles.timelineSteps}>
          {steps.map((step, index) => (<StepNode key={step.id} step={step} index={index} totalSteps={steps.length} scrollYProgress={scrollYProgress}/>))}
        </div>
      </div>
    </div>);
}
