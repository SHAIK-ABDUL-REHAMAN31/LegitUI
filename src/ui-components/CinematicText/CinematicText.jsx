...
const CinematicTextSpan = ({ char, index, total, scrollYProgress, activeColor, inactiveColor, }) => {
    ...
    return (<motion.span className={styles.letter} style={{
            opacity,
            y,
            rotateX,
            color,
...
export const CinematicText = ({ text = "Cinematic", subtitle = "Letter by Letter — Framer Motion", activeColor = "#000000ff", inactiveColor = "#ffffffff", fontSize = "clamp(3rem, 10vw, 9rem)", }) => {
    ...
    return (<div ref={containerRef} className={styles.container}>
            {/* Top decorative rule */}
            <motion.div className={styles.topRule} style={{
            scaleX: topRuleScale,
            opacity: topRuleOpacity,
        }}/>

            {/* Letter-by-letter heading */}
            <h1 className={styles.heading} style={{ fontSize }}>
                {chars.map((char, i) => (<CinematicTextSpan key={i} char={char} index={i} total={chars.length} scrollYProgress={smoothProgress} activeColor={activeColor} inactiveColor={inactiveColor}/>))}
            </h1>
            ...
            {/* Subtitle */}
            <motion.p className={styles.subtitle} style={{
