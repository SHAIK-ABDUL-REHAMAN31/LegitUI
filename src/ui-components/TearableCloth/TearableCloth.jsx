...
class Point {
    ...
    row;
    col;
    constructor(x, y, pinned, row, col) {
        this.x = x;
        this.y = y;
        this.px = x;
        this.py = y;
...
export const TearableCloth = ({ children, gridSpacing = 14, gravity = 0.22, influenceRadius = 40, tearSensitivity = 1.7, damping = 0.98, solverIterations = 5, clothHeightPercent = 80, }) => {
    ...
    return (<div className={styles.container} ref={containerRef}>
      ...
      <canvas className={`${styles.canvasLayer} ${isMouseDown ? styles.canvasGrabbing : ''}`} ref={canvasRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUpOrLeave} onMouseLeave={handleMouseUpOrLeave} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ pointerEvents: isFullyTorn ? 'none' : 'auto' }}/>

      {/* Reload button — top-right corner */}
      <button className={styles.reloadBtn} onClick={handleReset} title="Reset cloth">
        <RotateCcw size={16}/>
      </button>

      {!isTearingDone && (<div className={styles.hint}>
