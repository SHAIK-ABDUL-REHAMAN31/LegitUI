...
export default function CurvedTypographyGallery({ items = defaultItems, className = "" }) {
    ...
    useEffect(() => {
        ...
        const updateCards = (currentRot, entranceProgress = 1) => {
        ...
        const ctx = gsap.context(() => {
            ...
            gsap.fromTo(`.${styles.svgWrapper}`, { opacity: 0, scale: 0.8 }, { opacity: 0.35, scale: 1, duration: 2.0, ease: "power4.out", delay: 0.1 });
        ...
        <div ref={galleryRef} className={styles.gallery} style={{
          ...
          {items.map((item, i) => {
            ...
            return (<div key={item.id} className={styles.cardContainer} style={{
             ...
             className={styles.card} onMouseEnter={(e) => {
