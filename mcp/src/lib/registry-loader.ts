...
import { readFileSync, existsSync, readdirSync, readFile } from "node:fs";
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (existsSync(pkgPath)) {
      try {
        ...
        if (pkg.name === "legitui") return dir;
      } catch {
        // skip
      }
    }
...
async function fetchRemoteRegistry(url: string): Promise<ComponentRegistry> {
...
function loadLocalRegistry(): ComponentRegistry {
  const registryPath = join(PROJECT_ROOT, "public", "registry", "index.json");

  if (!existsSync(registryPath)) {
    throw new Error(
      `Registry not found at ${registryPath}. Run 'npm run registry:build' first.`
    );
  }
...
function normalizeRegistry(data: any): ComponentRegistry {
  const components: LegitUIComponent[] = (data.components || []).map(
    (c: any) => ({
      slug: c.slug,
      name: c.name,
      folder: c.folder || "",
...
export async function searchComponents(
  ...
  const scored = registry.components.map((c) => {
    ...
    if (deps.some((d) => d.includes(q))) score += 20;
    ...
    if (words.length > 1) {
      for (const word of words) {
        ...
        if (tags.some((t) => t.includes(word))) score += 12;
        if (cat.includes(word)) score += 10;
        if (deps.some((d) => d.includes(word))) score += 8;
...
export async function listByCategory(
  ...
  if (!category) return registry.components;

  const cat = category.toLowerCase();
  return registry.components.filter(
    (c) => c.category.toLowerCase() === cat || c.category.toLowerCase().replace(/\s+/g, "") === cat
  );
}
...
export async function getComponentSource(
  ...
  if (component.folder && component.fileName) {
    const ext =
      variant === "tsx" ? ".tsx" : variant === "jsx" ? ".jsx" : ".module.css";
    const filePath = join(
      PROJECT_ROOT,
      "src",
      "ui-components",
  ...
  if (existsSync(jsonPath)) {
    try {
      ...
      const fallback =
        match ||
        files.find((f) =>
          exts.some((ext) => f.name.toLowerCase().endsWith(ext.toLowerCase()))
        );
