...
export const definition = {
  ...
  description:
    "Install a LegitUI component directly into the developer's project " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      method: {
        ...
        enum: ["shadcn", "jsrepo"],
        description:
          "Install method: shadcn (recommended) or jsrepo. Default: shadcn",
      },
      target_dir: {
        type: "string",
        description:
          "Target directory for component files. Default: src/components/ui/",
      },
      dry_run: {
        type: "boolean",
        description:
          "If true, show the command that would run without executing it (default: false)",
      },
    },
    required: ["slug"],
  },
...
export async function handler(args: Record<string, unknown>): Promise<string> {
  ...
  const method = (args.method as string) || "shadcn";
  ...
  const componentName =
    component.fileName || component.name.replace(/\s+/g, "");
