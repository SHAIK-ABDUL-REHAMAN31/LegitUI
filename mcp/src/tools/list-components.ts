...
import {
  listByCategory,
  getCategories,
  type LegitUIComponent,
} from "../lib/registry-loader.js";

export const definition = {
  name: "list_components",
  description:
    "List all available LegitUI components. Optionally filter by category. " +
  ...
  inputSchema: {
    ...
    properties: {
      category: {
        type: "string",
        description:
          "Filter by category: Backgrounds | TextAnimations | Animations | " +
...
function formatComponent(c: LegitUIComponent): string {
...
export async function handler(args: Record<string, unknown>): Promise<string> {
  ...
  // Group by category
  const grouped = new Map<string, LegitUIComponent[]>();
