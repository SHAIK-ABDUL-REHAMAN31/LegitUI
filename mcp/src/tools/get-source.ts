...
export const definition = {
  ...
  description:
    "Get the full source code of a LegitUI component. " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      variant: {
        ...
        enum: ["tsx", "jsx", "css"],
        description:
          "Source variant: tsx (TypeScript), jsx (JavaScript), css (styles). Default: tsx",
      },
    },
    required: ["slug"],
  },
...
export async function handler(args: Record<string, unknown>): Promise<string> {
  ...
  const variant = (args.variant as "tsx" | "jsx" | "css") || "tsx";
  ...
  if (!source && (variant === "tsx" || variant === "jsx")) {
    ...
    if (source) actualVariant = fallbackVariant;
  }

  if (!source) {
    return (
      `Source file not found for "${slug}" (variant: ${variant}).\n\n` +
  ...
  const langMap: Record<string, string> = { tsx: "tsx", jsx: "jsx", css: "css" };
  const note =
